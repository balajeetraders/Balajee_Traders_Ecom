...
import { Package, CheckCircle, ArrowRight, Loader2, AlertCircle, Star } from 'lucide-react';
import gsap from 'gsap';
import { supabase } from '../lib/supabase';
import { useAuth } from '../context/AuthContext';
import { Order, CartItem } from '../types';
import ReviewModal from '../components/ReviewModal';
import { hasReviewedOrder } from '../lib/reviewService';
      ...
      try {
        ...
        const formattedOrders: Order[] = (data || []).map((order: any) => ({
          ...
          items: order.items.map((item: any) => ({
            ...
            selectedColor: item.variant?.split('/')[0]?.trim(),
            image: item.product?.image || 'https://images.unsplash.com/photo-1555041469-a586c61ea9bc?auto=format&fit=crop&q=80&w=800',
            category: 'Archived', room: 'Archived', style: 'Archived',
            material: 'Archived', description: '', rating: 5, reviewCount: 0,
          } as CartItem)),
        }));
        ...
        if (formattedOrders.length > 0) {
          const checks = await Promise.all(
            formattedOrders.map(o => hasReviewedOrder(o.id, user.id).then(has => has ? o.id : null))
          );
  ...
  useEffect(() => {
    if (!loading && orders.length > 0) {
      const ctx = gsap.context(() => {
        gsap.from('.order-card', { y: 30, opacity: 0, stagger: 0.1, duration: 0.8, ease: 'power3.out' });
      }, containerRef);
      return () => ctx.revert();
    }
  }, [loading, orders]);
    ...
    return (
      <div className="min-h-screen pt-40 pb-20 bg-stone-50 text-center px-6">
        <div className="max-w-md mx-auto space-y-8">
          ...
          <p className="text-stone-500">Please sign in to view your acquisition history.</p>
    ...
    <div ref={containerRef} className="pt-24 pb-32 md:pt-40 md:pb-40 bg-stone-50 min-h-screen">
        ...
        <div className="max-w-5xl mx-auto space-y-12">
          ...
          <div className="space-y-4">
            ...
            <h1 className="text-4xl md:text-6xl font-serif text-stone-900 leading-none">
              Your <span className="italic font-light opacity-50">Collection</span>
            </h1>
          </div>
          ...
          {orders.length === 0 ? (
            <div className="py-20 flex flex-col items-center justify-center text-center space-y-8 bg-white rounded-[3rem] shadow-sm border border-stone-100">
              ...
              <div className="space-y-2">
                ...
                <p className="text-stone-400 text-sm max-w-xs mx-auto">Your architectural journey begins in the shop.</p>
              </div>
              ...
              {orders.map((order) => (
                <div key={order.id} className="order-card bg-white rounded-[2rem] p-6 md:p-8 shadow-sm border border-stone-100 space-y-8">
                  ...
                  <div className="pt-4 flex flex-wrap items-center justify-between gap-3">
                    ...
                    {order.status?.toLowerCase() === 'delivered' && !reviewedIds.has(order.id) && (
                      <button
                        onClick={() => setReviewTarget(order.id)}
                        className="flex items-center gap-2 px-5 py-2.5 bg-amber-50 text-amber-700 hover:bg-amber-100 rounded-full text-[10px] font-black uppercase tracking-widest transition-all border border-amber-200"
                      >
                    ...
                    {reviewedIds.has(order.id) && (
                      <span className="flex items-center gap-2 px-4 py-2 bg-green-50 text-green-700 rounded-full text-[9px] font-black uppercase tracking-widest border border-green-100">
      ...
      {reviewTarget && (
        <ReviewModal
          ...
          onSubmitted={() => {
            setReviewedIds(prev => new Set([...prev, reviewTarget!]));
