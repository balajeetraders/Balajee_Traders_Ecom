...
const Navbar: React.FC<NavbarProps> = ({ cartCount, wishlistCount, onOpenCart }) => {
  ...
  return (
    <header
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 ${isScrolled ? 'bg-white shadow-sm py-4' : 'bg-transparent py-6 md:py-8'
        }`}
    >
            ...
            {navItems.map((item) => {
              ...
              return (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`group relative text-[11px] uppercase tracking-[0.4em] font-black transition-all text-stone-900 ${!isActive ? 'opacity-40 hover:opacity-100' : 'opacity-100'
                    }`}
                >
                  {item.name}
                  <span className={`absolute -bottom-1 left-0 h-[2px] bg-stone-900 transition-all duration-500 ${isActive ? 'w-full' : 'w-0 group-hover:w-full opacity-30'
                    }`}></span>
                </Link>
              );
            })}
